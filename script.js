...
function isRealtimeTypingFieldActive() {
  ...
  return ['intel-alert-title', 'intel-alert-content', 'esp-live-message', 'stealth-input'].includes(active.id);
}

function shouldDeferRealtimeRenderForPage(page) {
  if (!isRealtimeTypingFieldActive()) return false;
  return page === 'intelCenter' || page === 'espionage' || page === 'stealthChat';
}
...
function isStealthPairAllowed(creatorRole, targetRole) {
  const fromRole = String(creatorRole || 'member');
  const toRole = String(targetRole || 'member');
  if (fromRole !== 'member') return true;
  return toRole === 'member';
}
...
function ensureCoreUsers() {
  ...
  requiredUsers.forEach((seed) => {
    ...
    if (!existing.password) existing.password = seed.password;
...
function mergeLocalUsersIntoRemoteState(remoteUsersRaw, localUsersRaw) {
  ...
  localUsers.forEach((localUser) => {
    ...
    const localPassword = String(localUser.password || '');
    const remotePassword = String(remoteUser.password || '');

    // Keep custom local password when remote still has default or empty.
    if (localPassword && localPassword !== remotePassword) {
      if (!remotePassword || remotePassword === DEFAULT_PASSWORD) {
        remoteUser.password = localPassword;
        localMerged = true;
      }
    }
...
function connectPresenceSocket() {
  ...
  socket.addEventListener('open', () => {
    ...
    presenceInitFallbackTimer = setTimeout(() => {
      if (!session || presenceSocket !== socket || !presenceSocketConnected || remoteStateInitialized) return;
  ...
  socket.addEventListener('message', (event) => {
    ...
    if (payload.type === 'app_state_sync' && payload.state && typeof payload.state === 'object' && !Array.isArray(payload.state)) {
      ...
      const firstStateSyncForThisConnection = !remoteStateInitialized;
      const mergedStateResult = firstStateSyncForThisConnection
        ? mergeIncomingAppStateWithLocal(payload.state, localBefore)
        : { state: payload.state, changed: false };
      ...
      } else if (firstStateSyncForThisConnection && mergedStateResult.changed) {
        sendPresenceSocketMessage({
          type: 'app_state_sync',
          origin: SYNC_CLIENT_ID,
          merge: true,
...
function getPresenceMapForOnline() {
  ...
  if (presenceSocketConnected && sharedPresenceMap && typeof sharedPresenceMap === 'object' && !Array.isArray(sharedPresenceMap)) {
    const sharedHasEntries = Object.keys(sharedPresenceMap).length > 0;
    const localHasEntries = Object.keys(localMap).length > 0;
    if (sharedHasEntries || !localHasEntries) {
...
function renderLogs() {
  if (!canViewLogs()) {
    renderAccessDenied('Apenas Superadmin e Inteligencia TP podem acessar os registros.');
    return;
  }
  ...
  const accessRows = users
    ...
    .map((user) => {
      const online = onlineMap.get(user.username);
      const loginAt = Number(user.lastLoginAt) || 0;
...
function canUseStealthAnonymousMode() {
  return Boolean(session && (isAdminRole(session.role) || session.role === 'inteligencia'));
}

function getStealthMessageAuthorLabel(message) {
  if (message && message.anonymous) return 'Anonimo';
  return message && message.author ? message.author : 'sistema';
}
...
function renderStealthChat() {
  ...
  if (!activeSession) {
    ...
    createForm.addEventListener('submit', (event) => {
      ...
      showNotification(`Chat irrastreavel criado com ${result.target}.`, 'success');
  ...
  const canSendAnonymous = canUseStealthAnonymousMode();
  ...
  els.content.innerHTML = `
    <section class="panel">
      ...
      <form id="stealth-form" class="chat-input-area" style="margin-top: 12px;" data-session-id="${activeSession.id}">
        ${canSendAnonymous
          ? `
            <select id="stealth-send-mode" style="max-width: 220px;">
              <option value="named">Com nome</option>
              <option value="anonymous">Anonimo (admin)</option>
            </select>
          `
          : ''}
  ...
  const form = document.getElementById('stealth-form');
  const input = document.getElementById('stealth-input');
  const sendModeSelect = document.getElementById('stealth-send-mode');
  ...
  form.addEventListener('submit', (event) => {
    ...
    if (!text) return;
    const sendAsAnonymous = Boolean(sendModeSelect && sendModeSelect.value === 'anonymous' && canSendAnonymous);
