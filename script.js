...
const MOBILE_REALTIME_EXTRA_MS = 900;
...
function canUserSeeOwnWalletChart(user) {
  if (!user || user.status !== 'active') return false;
  if (isRoleAboveMember(user.role)) return true;
  return canUserHaveWalletChart(user);
}
...
function renderDashboard() {
  ...
  const hasPersonalChart = canUserSeeOwnWalletChart(user);
  ...
  const personalChartHtml = hasPersonalChart
    ...
    : '<div class="empty-state">Seu perfil nao possui grafico ativo. Esther pode liberar grafico para membros.</div>';
