...
function isStealthPairAllowed(creatorRole, targetRole) {
  ...
  const involvesMember = fromRole === 'member' || toRole === 'member';
  if (!involvesMember) return true;
  return fromRole === 'member' && toRole === 'member';
}
...
function createStealthSessionWithUser(targetUsername) {
  ...
  if (!isStealthPairAllowed(me ? me.role : session.role, target.role)) {
    return { ok: false, message: 'No chat irrastreavel, membro so fala com membro.' };
  }
...
function renderStealthChat() {
  ...
  if (me && peerUser && !isStealthPairAllowed(me.role, peerUser.role)) {
    destroyStealthChat(false);
    showNotification('Sessao encerrada: no chat irrastreavel, membro so fala com membro.', 'warning');
    renderSidebar();
    renderStealthChat();
    return;
  }
